function getRotationPar(center, vector, t) {
    ...
    // u, v, w is vector[0], vector[1], vector[2] respectively
    // a, b, c is centre[0], centre[1], centre[2] respectively

    var u_u = vector[0] * vector[0];
    var v_v = vector[1] * vector[1];
    var w_w = vector[2] * vector[2];

    var v_v_p_w_w = (v_v + w_w);//1
    var u_u_p_w_w = (u_u + w_w);//2
    var u_u_p_v_v = (u_u + v_v);//3
...
function translateObj(vector, obj) {
    ...
    //first move the objects centre
    translate(vector, obj.center);

    //then move each of the object's points
